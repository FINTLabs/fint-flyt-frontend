...
describe('Testing creating new and editing configurations from integration overview', () => {
    ...
    beforeEach(() => {
        ...
        cy.intercept('GET', '**/metadata?kildeapplikasjonId=1&bareSisteVersjoner=false', {
            fixture: 'metadata.json',
        }).as('getMetadata');
        cy.intercept('GET', '**/metadata?kildeapplikasjonId=2&bareSisteVersjoner=false', {
            fixture: 'metadata.json',
        }).as('getMetadata');
        cy.intercept('GET', '**/metadata?kildeapplikasjonId=3&bareSisteVersjoner=false', {
            fixture: 'metadata.json',
        }).as('getMetadata');
        cy.intercept('GET', '**/metadata?kildeapplikasjonId=4&bareSisteVersjoner=false', {
            fixture: 'metadata.json',
        }).as('getMetadata');
        cy.intercept('GET', '**/metadata/11/instans-metadata', {
            fixture: 'instansMetadata.json',
        }).as('getInstansMetadata');
        cy.intercept('GET', '**/metadata/1/instans-metadata', {
            fixture: 'instansMetadata.json',
        }).as('getInstansMetadata');
        cy.intercept(
            'GET',
        ...
        cy.intercept('GET', '**/konfigurasjoner/5?ekskluderMapping=false', {
            fixture: 'config.json',
        }).as('getConfig');
        cy.intercept('GET', '**/konfigurasjoner/4?ekskluderMapping=true', {
            fixture: 'config.json',
        }).as('getConfig');
        cy.intercept('GET', '**/konfigurasjoner/7?ekskluderMapping=false', {
    ...
    it('should navigate to create new configuration based on existing completed version', () => {
        ...
        cy.get(
            '#panel-1-action-toggle > .navds-popover > dl.navds-dropdown__list > :nth-child(2) > .navds-dropdown__item'
        ).click();
