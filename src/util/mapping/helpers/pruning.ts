...
function filterEntryNotNull<T>(entry: {
    key: string;
    value: T | null;
}): entry is { key: string; value: T } {
...
function pruneCollectionMapping<T extends IValueMapping | IObjectMapping>(
    collectionMapping: ICollectionMapping<T>,
    pruningFunction: (value: T) => T | null
): ICollectionMapping<T> | null {
    const elementMappings: T[] = collectionMapping.elementMappings
        .map(pruningFunction)
        .filter(filterNotNull);
    const fromCollectionMappings: IFromCollectionMapping<T>[] =
        collectionMapping.fromCollectionMappings
            .map((fromCollectionMapping) =>
                pruneFromCollectionMapping(fromCollectionMapping, pruningFunction)
            )
            .filter(filterNotNull);
...
function pruneFromCollectionMapping<T extends IValueMapping | IObjectMapping>(
    fromCollectionMapping: IFromCollectionMapping<T>,
    pruningFunction: (value: T) => T | null
): IFromCollectionMapping<T> | null {
    const elementMapping: T | null = pruningFunction(fromCollectionMapping.elementMapping);
    return elementMapping
        ? {
              instanceCollectionReferencesOrdered:
                  fromCollectionMapping.instanceCollectionReferencesOrdered,
              elementMapping,
          }
        : null;
}
...
export function pruneObjectMapping(objectMapping: IObjectMapping): IObjectMapping | null {
    ...
    const valueCollectionMappingPerKey: Record<
        string,
        ICollectionMapping<IValueMapping>
    > = objectMapping.valueCollectionMappingPerKey
        ? pruneRecord(
              objectMapping.valueCollectionMappingPerKey,
              (collectionMapping: ICollectionMapping<IValueMapping>) =>
                  pruneCollectionMapping(collectionMapping, pruneValueMapping)
          )
        : {};

    const objectCollectionMappingPerKey: Record<
        string,
        ICollectionMapping<IObjectMapping>
    > = objectMapping.objectCollectionMappingPerKey
        ? pruneRecord(
              objectMapping.objectCollectionMappingPerKey,
              (collectionMapping: ICollectionMapping<IObjectMapping>) =>
                  pruneCollectionMapping(collectionMapping, pruneObjectMapping)
          )
        : {};

    const objectMappingPerKey: Record<string, IObjectMapping> = objectMapping.objectMappingPerKey
        ? pruneRecord(objectMapping.objectMappingPerKey, pruneObjectMapping)
        : {};
