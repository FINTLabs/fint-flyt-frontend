...
test('It should show error messages correctly', () => {
    expect(errorStringReplace(testStrings[0], errorArgs)).toEqual(
        "Valideringsfeil i mottak av instans, 'foo bar'"
    );
    expect(errorStringReplace(testStrings[1], errorArgs)).toEqual(
        'Mangler felt ‘qux’ i instans, som kreves for å sette felt i sak'
    );
    expect(errorStringReplace(testStrings[2], resultingErrorArgs)).toEqual(
        "Instansen ble avvist av destinasjon med følgende feilmelding: 'On a scale of 1-10 my friend, you're f'ed'"
    );
    expect(errorStringReplace(testStrings[3], resultingErrorArgs)).toEqual(
        "Instans avvist, 'Ingen støtte for dette domenet'"
    );
    expect(errorStringReplace(testStrings[4], errorArgs)).toEqual(
        "Feil under opplasting av fil, filnavn: 'fil.docx' med mediatype: 'app/docx'"
    );
    expect(errorStringReplace(testStrings[5], errorArgs)).toEqual(
        "Feil under verdikonvertering, finner ikke nøkkel: 'image/jpg', i verdikonvertering med ID: 5"
    );
});
