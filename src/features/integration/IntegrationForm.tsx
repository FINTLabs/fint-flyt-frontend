...
} from "../../context/SourceApplicationContext";
...
export const IntegrationForm: React.FunctionComponent<
	RouteComponentProps<Props>
> = () => {
	const classes = useStyles();
	const history = useHistory();
	const { t } = useTranslation("translations", {
		keyPrefix: "components.integrationForm",
	});
	const {
		setSelectedMetadata,
	...
	const confirm = () => {
		const selectedForm = allMetadata.find(
			(md: IIntegrationMetadata) =>
				md.sourceApplicationIntegrationId === sourceApplicationIntegrationId
		);
		if (
			!destination ||
			!sourceApplicationId ||
		...
		const newIntegration: IIntegration = toIntegration(
			formConfiguration,
			IntegrationState.DEACTIVATED
		);
	...
	return (
		<InformationTemplate id={"new"} keyPrefix={"components.integrationForm"}>
			<FormGroup id="integration-form" className={classes.panelContainer}>
				<Box>
					<h2 className={classes.title2} id="integration-form-settings-header">
						{t("header")}
					</h2>
					<Box className={classes.incomingWrapper}>
						...
						<Box sx={{ display: "flex", mt: 2 }}>
							<Autocomplete
								...
								options={
									sourceApplication && availableForms
										? availableForms
					...
					<Typography id={"form-error-msg"} color={"error"}>
						{!sourceApplicationId ||
						!sourceApplicationIntegrationId ||
						!destination
							? error
