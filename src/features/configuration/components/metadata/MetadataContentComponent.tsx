    ...
    return (
        <>
            <HStack align={'center'} gap={'2'}>
                ...
                {props.displayName && <Heading size={'xsmall'}>{props.displayName}</Heading>}
            </HStack>
            <VStack gap={'1'}>
                {props.content.instanceValueMetadata.map(
                    (valueMetadata: IInstanceValueMetadata) => {
                ...
                {props.content.instanceObjectCollectionMetadata.map(
                    (objectCollectionMetadata: IInstanceObjectCollectionMetadata) => {
                        const reference: string = props.keyToReferenceFunction(
                    ...
                    <div style={{ paddingLeft: paddingPerDepth, borderLeft: '1px solid gray' }}>
                        <MetadataContentComponent
                            ...
                            keyToReferenceFunction={(key: string) =>
                                props.keyToReferenceFunction(key)
                            }
                        />
                    </div>
                </div>
