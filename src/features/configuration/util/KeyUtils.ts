...
export function findFromCollectionMappingAbsoluteKeys(absoluteKey: string): string[] {
    ...
    if (match) {
        const absoluteKeyToLastIndexOfMatch: string = absoluteKey.slice(
            0,
            match.index + match[0].length
        );
        const absoluteKeyFromLastIndexOfMatch: string = absoluteKey.slice(
            match.index + match[0].length
        );
        return [
            match[0],
            ...findFromCollectionMappingAbsoluteKeys(absoluteKeyFromLastIndexOfMatch).map(
                (matchedFromCollectionMappingAbsoluteKey: string) =>
...
export function isOutsideCollectionEditContext(
    absoluteKey: string,
    editContextAbsoluteKey: string
): boolean {
    return (
        !absoluteKey.startsWith(editContextAbsoluteKey) ||
