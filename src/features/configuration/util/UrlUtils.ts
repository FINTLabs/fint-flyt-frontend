...
export function createValueRefPerAbsoluteKey(
    sourceUrlBuilders: IUrlBuilder[],
    absoluteKey: string
): Record<string, string> {
    return sourceUrlBuilders
        .map((urlBuilder) => [
            ...(urlBuilder.valueRefPerRequestParamKey
                ? Object.values(urlBuilder.valueRefPerRequestParamKey)
                : []),
            ...(urlBuilder.valueRefPerPathParamKey
                ? Object.values(urlBuilder.valueRefPerPathParamKey)
                : []),
        ])
        .reduce<Record<string, string>>(
            (valueRefPerAbsoluteKey: Record<string, string>, currentValue) => {
                currentValue.forEach(
                    (valueRef) =>
                        (valueRefPerAbsoluteKey[getAbsoluteKeyFromValueRef(valueRef, absoluteKey)] =
                            valueRef)
                );
...
export function createSource(
    ...
    Object.entries(valueRefPerRequestParamKey).forEach(([requestParamKey, valueRef]) => {
        params[requestParamKey] = valuePerValueRef[valueRef];
    });

    if (Object.keys(valueRefPerPathParamKey).length > 0) {
        url = url.replaceAll(pathParamKeyRegExp, (substring: string) => {
