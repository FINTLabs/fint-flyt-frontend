...
import {
	IIntegrationPatch,
	IntegrationState,
} from "../integration/types/Integration";
...
const ConfigurationForm: RouteComponent = () => {
	...
	const { completed, setCompleted, resetConfigurationContext } =
	...
	const [version, setVersion] = React.useState<string>(
		selectedMetadata ? String(selectedMetadata.version) : ""
	);
	...
	useEffect(() => {
		...
		if (configuration?.integrationMetadataId) {
			getInstanceElementMetadata(
				configuration.integrationMetadataId.toString()
			);
		}
		return () => {
			resetIntegrationContext();
	...
	const handleChange = (
		...
		const integrationMetadata: IIntegrationMetadata[] =
			availableVersions.filter((metadata) => metadata.version === version);
		setSelectedMetadata(integrationMetadata[0]);
		if (integrationMetadata[0].id) {
			methods.setValue(
				"integrationMetadataId",
				Number(integrationMetadata[0].id)
			);
		...
		IntegrationRepository.updateIntegration(integrationId, patch)
			.then((response) => {
				...
				console.log(
					...
					response.data.activeConfigurationId
				);
			})
			.catch((e) => {
	...
	const availableVersions: IIntegrationMetadata[] = allMetadata.filter((md) => {
		return (
			md.sourceApplicationId === selectedMetadata?.sourceApplicationId &&
			md.sourceApplicationIntegrationId ===
				selectedMetadata.sourceApplicationIntegrationId
		);
	});
					...
					<FormProvider {...methods}>
						...
						>
							<Box className={classes.configurationBox} sx={{ m: 1 }}>
								...
								<Box
									sx={{ mb: 1, width: (theme: Theme) => theme.spacing(100) }}
								>
										...
										>
											...
											>
												{availableVersions.map((md, index) => {
													return (
														<MenuItem key={index} value={md.version}>
															Versjon {md.version}
														</MenuItem>
													);
												})}
