...
const IntegrationPanel: React.FunctionComponent<Props> = (props: Props) => {
	const { t, i18n } = useTranslation("translations", {
		keyPrefix: "pages.integrationOverview",
	});
	...
	const { allMetadata, getAllMetadata, getInstanceElementMetadata } =
		useContext(SourceApplicationContext);
	const [version, setVersion] = useState("null");
	const [activeVersion, setActiveVersion] = useState<string>("");
	const [anchorEl, setAnchorEl] = React.useState<null | HTMLElement>(null);
	const [anchorSubEl, setAnchorSubEl] = React.useState<null | HTMLElement>(
		null
	);
	...
	let versionsToActivate: ISelect[] = [
		{ value: "null", label: "velg aktiv versjon" },
	];
	...
	useEffect(() => {
		...
		getVersionForActiveConfig(
			existingIntegration?.activeConfigurationId
				? existingIntegration.activeConfigurationId
				: undefined
		);
	...
	const columns: GridColDef[] = [
		...
		},
		{
			field: "version",
			type: "number",
			headerName: t("table.columns.version"),
			minWidth: 100,
			flex: 0.5,
		},
		{
			field: "comment",
			type: "string",
			headerName: t("table.columns.comment"),
			minWidth: 250,
			flex: 1,
	...
	async function handleNewOrEditConfigClick(
		id: number | string,
		version?: unknown
	) {
		await ConfigurationRepository.getConfigurationById(id.toString(), false)
			.then(async (response) => {
				...
				const usedVersionMetadata = allMetadata.filter(
					(md) => md.id === data.integrationMetadataId
				);
				setSelectedMetadata(
					usedVersionMetadata.length > 0 ? usedVersionMetadata[0] : undefined
				);
				if (version) {
					data.id = undefined;
	...
	function EditButtonToggle(props: GridCellParams["row"]) {
		...
		return (
			<Button
				...
				variant="contained"
				onClick={() => {
					handleNewOrEditConfigClick(props.row.id).then(() =>
	...
	const activateConfiguration = (configurationId: string) => {
		...
		if (existingIntegration?.id) {
			IntegrationRepository.updateIntegration(existingIntegration?.id, patch)
				.then((response) => {
					console.log(
						"updated integration: ",
	...
	return (
		<Box>
			...
			>
				<DialogTitle id="alert-dialog-title">{t("dialog.header")}</DialogTitle>
				<DialogContent>
			...
			<Box sx={{ mb: 2 }} className={classes.integrationWrapper}>
				{infoTypography(
					"id:",
					existingIntegration?.id ? existingIntegration.id : ""
				)}
				{infoTypography(
			...
			>
				...
				>
					<Button
						...
						onClick={() => {
							const selectedForm = allMetadata.filter(
								(md) =>
				...
				<MenuItem disableGutters={true} divider={true} dense={true}>
					<Button
						disabled={
							!completedConfigurations || completedConfigurations.length === 0
						}
						id="positioned-button"
						...
						{completedConfigurations &&
							[...completedConfigurations]
								...
								.map((config: IConfiguration, index: number) => {
									return (
										<MenuItem
											onClick={handleNewConfigSubClose}
											disableGutters={true}
											divider={true}
											dense={true}
											key={index}
