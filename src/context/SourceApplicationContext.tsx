...
const contextDefaultValues: SourceApplicationContextState = {
	availableForms: [{ value: "null", label: "Velg skjemaleverandør først" }],
...
const SourceApplicationProvider = ({ children }: ContextProps) => {
	...
	const [allMetadata, setAllMetadata] = useState<IIntegrationMetadata[]>(
	...
	const getAvailableForms = async () => {
		try {
			const sourceAppId =
				sourceApplication !== undefined ? sourceApplication.toString() : "2";
			...
			const selectables: ISelect[] = data.map(
				(value: IIntegrationMetadata) => ({
